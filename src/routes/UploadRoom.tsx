...
export default function UploadRoom() {
    ...
    const { isLoading : isAmenitiesLoading, data : amenities} = useQuery<IAmenity[]>({
        queryKey: ["amenities"],
        queryFn: getAmenities
    });
    const {isLoading : isCategoriesLoading, data : categories} = useQuery<ICategory[]>({
        queryKey: ["categories"],
                        ...
                        <FormControl>
                            ...
                            <Grid templateColumns={"1fr 1fr"} gap={5}>
                                {amenities?.map((amenity) => (
                                    <Box key={amenity.id}>
                                        <Checkbox value={amenity.id} {...register("amenities", {required : true})}>{amenity.name}</Checkbox>
